...
int main(int argc, char* argv[])
{
    ...
    while (true)
    {
        ...
        if (connected())
        {
            ...
            // To proceed in this task I've decided to divide the request line in 3 tokens: method, request target and http-version
            // http://www.w3.org/Protocols/rfc2616/rfc2616-sec5.html
            // create varaible for the tokens
            char* token = strtok(line, " ");
            
            //declaring an array for the first token: method
            char method[strlen(token)+1];
            
            //coping method into array
            memset(method, '\0', sizeof(method));
            strcpy(method, token);
            
            //declaring array and copying for the second token: request URI
            token = strtok(NULL, " ");
            char request_target[strlen(token +1)];
            memset(request_target, '0', sizeof(request_target));
            strcpy(request_target, token);
            
            //token for the 3rd part: http-version
            token = strtok(NULL, " ");
            int httpver = strlen(token);
            
            char httpv[httpver - 1];
            strncpy (httpv, token, 8);
            
            //check if request-target is valid per standard specifications
            //http://www.w3.org/Protocols/rfc2616/rfc2616-sec5.html
            if (strtok(NULL, " ") != NULL)
            {
                error(400);
            ...
            //check if request-target doesn't begin with /
            ...
            const char quote = '"';
            if (strchr(request_target, quote) != 0)
            {
                error(400);
            }
            
            //if http version is not http/1.1
            if(strcasecmp(httpv, "http/1.1") != 0)
            {
                error(505);
            }
            
            //if absoluthe-path does not contains a file extension (.)
            const char p = '.';
            if (strchr(request_target, p) == 0)
            {
                error(501);
            }
            ...
            const char qm = '?';
            char* qmquery;
            qmquery = strchr(request_target, qm);                       
            ...
            if (qmquery != 0)
            {
                //if there's no query after '?'
                if(strcmp(qmquery, "?") == 0)
                {
                    //create a buffer
                    char q_buffer[1];
                    q_buffer[0] = '\0';
                    strcpy(query, q_buffer);
                }
                
                //if there's a query after '?'
                else
                {
                    char q_buffer[strlen(qmquery)];
                    memset(q_buffer, '\0', sizeof(q_buffer));
                    strcpy(q_buffer,qmquery +1);
                    strcpy(query, q_buffer);
                }               
            }
            ...
            //setting the values for concatenating root to aboslute path later
            //lenght of the requested_target
            int rtlen = strlen(request_target);
            ...
            //lenght of absolute-path
            int abslen = (rtlen - qlen);
            
            //absolute_path array
            char abspath[abslen+1];
            memset(abspath, '\0', sizeof(abspath));
            strncpy(abspath, request_target, abslen);
            
            //path array
            char pathnoabs[strlen(root) +1];
            memset(pathnoabs, '\0', sizeof(pathnoabs));
            strcpy(pathnoabs, root);
            
            // TODO: concatenate root and absolute-path
            char path[strlen(pathnoabs) + strlen(abspath) +1];
            ...
            //extracting path extension
            char* dotextension;
            char likeabs[abslen + 1];
            memset(likeabs, '\0', sizeof(likeabs));
            strncpy(likeabs, request_target, abslen);
            dotextension = strchr(likeabs, p);
 
            // TODO: extract path's extension
            char extension[strlen(dotextension) - 1];
...
const char* lookup(const char* extension)
{
    // TODO  using strcasecmp to check the extensions.
